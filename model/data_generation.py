...
class Generator():
    def __init__(self, cohort_output, if_mort, if_admn, if_los, feat_cond, feat_lab, feat_proc, feat_med, impute, include_time=24, bucket=50, predW=0):
        self.impute = impute
    ...
    def generate_cond(self):
        cond = pd.read_csv(f"{DATA_PATH}/data/features/preproc_diag.csv.gz", compression='gzip', header=0, index_col=None)
    ...
    def generate_proc(self):
        proc = pd.read_csv(f"{DATA_PATH}/data/features/preproc_proc.csv.gz", compression='gzip', header=0, index_col=None)
        proc = proc[proc['hadm_id'].isin(self.data['hadm_id'])]
        proc[['start_days', 'dummy', 'start_hours']] = proc['proc_time_from_admit'].str.split(' ', n=2, expand=True)
    ...
    def generate_labs(self):
        chunksize = 10000000
        final = pd.DataFrame()
        for labs in tqdm(pd.read_csv(f"{DATA_PATH}/data/features/preproc_labs.csv.gz", compression='gzip', header=0, index_col=None, chunksize=chunksize)):
            labs = labs[labs['hadm_id'].isin(self.data['hadm_id'])]
            labs[['start_days', 'dummy', 'start_hours']] = labs['lab_time_from_admit'].str.split(' ', n=2, expand=True)
            labs[['start_hours', 'min', 'sec']] = labs['start_hours'].str.split(':', n=2, expand=True)
    ...
    def generate_meds(self):
        meds = pd.read_csv(f"{DATA_PATH}/data/features/preproc_med.csv.gz", compression='gzip', header=0, index_col=None)
    ...
    def create_Dict(self, meds, proc, labs, los):
        ...
        if(self.feat_lab):    
            with open(f"{DATA_PATH}/data/dict/labsVocab", 'wb') as fp:
