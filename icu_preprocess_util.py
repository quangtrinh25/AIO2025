...
def preproc_chart(dataset_path: str, cohort_path: str, time_col: str, dtypes: dict, usecols: list, max_chunks=None) -> pd.DataFrame:
    """Function for getting hosp observations pertaining to a pickled cohort. Function is structured to save memory when reading and transforming data."""
    ...
    # read module w/ custom params
    chunksize = 50000  # Giữ nguyên chunksize
    count = 0
    nitem = []
    nstay = []
    nrows = 0
    output_path = f"{os.path.splitext(dataset_path)[0]}_preproc.csv.gz"
    
    with tqdm(total=os.path.getsize(dataset_path) // chunksize + 1 if max_chunks is None else max_chunks, desc="Processing chart chunks") as pbar:
        for chunk in pd.read_csv(dataset_path, compression='gzip', usecols=usecols, dtype=dtypes, parse_dates=[time_col], chunksize=chunksize):
            count += 1
            ...
            chunk_merged = chunk_merged.dropna().drop_duplicates()
            
            # Lưu từng chunk vào file tạm
            temp_file = f"{os.path.join(os.path.dirname(output_path), 'temp_chunk_{count}.csv.gz')}"
            chunk_merged.to_csv(temp_file, compression='gzip', index=False)
            pbar.update(1)
            
            # Giới hạn số chunk nếu test
            if max_chunks and count >= max_chunks:
                print(f"Test mode: Stopped after {max_chunks} chunks.")
                break
    
    # Gộp các file tạm thành df_cohort
    temp_files = glob.glob(os.path.join(os.path.dirname(output_path), 'temp_chunk_*.csv.gz'))
    df_cohort = pd.concat([pd.read_csv(f, compression='gzip') for f in tqdm(temp_files, desc="Combining chart chunks")], ignore_index=True)
    df_cohort.to_csv(output_path, compression='gzip', index=False)
    
    # Xóa file tạm (có thể comment nếu muốn giữ)
    for f in temp_files:
        os.remove(f)
    
    # Print unique counts and value_counts, giữ nguyên cách in
